...
router.put("/Favourited/:userId", async (req, res) => {
  ...
  try {
    ...
    const result = await user.updateOne(filter, {
      $push: { Favourited: songId },
    });
    res.status(200).send({ success: true, msg: "Song added to Favourited" });
  } catch (error) {
...
router.put("/removeFavourited/:userId", async (req, res) => {
  const filter = { _id: req.params.userId };
  const songId = req.query;
  ...
  try {
    console.log(filter, songId);
    const result = await user.updateOne(filter, {
      $pull: { Favourited: songId },
    });
    res
      .status(200)
      .send({ success: true, msg: "Song removed from Favourited" });
  } catch (error) {
    res.status(400).send({ success: false, msg: error });
  }
});
